/*
东东超市兑换奖品 脚本�址：https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_blueCoin.js
感谢@yangtingxiao提供PR
更新时间：2020-12-24
支持京东多个账号
...
let coinToBeans = $.getdata('coinToBeans') || 20; //兑换多少数量的京豆（20或???1000），0表示不兑换，默认兑换20京豆，如需兑换???0改成20或???1000，或???'商品名称'(商品名称放到单引号内)即可
...
async function PrizeIndex() {
  ...
  // await smtg_materialPrizeIndex();//兑换酒类奖品，此兑换API与之前的兑换京豆类的不一致，故目前无法进???
  ...
  if (`${coinToBeans}` === '1000') {
    if (prizeList[1] && prizeList[1].beanType === 'BeanPackage') {
      console.log(`查询???${prizeList[1].title}ID成功，ID:${prizeList[1].prizeId}`)
      $.title = prizeList[1].title;
      $.blueCost = prizeList[1].blueCost;
    } else {
      console.log(`查询???1000京豆ID失败`)
      $.beanerr = `东哥今天不给换`;
      return ;
    }
    ...
    if ($.totalBlue > $.blueCost) {
      ...
      console.log(`兑换失败,您目前蓝???${$.totalBlue}???,不足以兑???${$.title}所需???${$.blueCost}个`);
      $.beanerr = `兑换失败,您目前蓝???${$.totalBlue}???,不足以兑???${$.title}所需???${$.blueCost}个`;
    }
  } else if (`${coinToBeans}` === '20') {
    if (prizeList[0] && prizeList[0].beanType === 'Bean') {
      console.log(`查询???${prizeList[0].title}ID成功，ID:${prizeList[0].prizeId}`)
    ...
    if (prizeId) {
      if (prizeList[i].inStock === 506 || prizeList[i].inStock === -1) {
        console.log(`失败，您输入设置???${coinToBeans}领光了，请明天再来`);
        $.beanerr = `失败，您输入设置???${coinToBeans}领光了，请明天再来`;
        return ;
      }
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (safeGet(data)) {
            ...
            if ($.data.data.bizCode !== 0) {
              $.beanerr = `${$.data.data.bizMsg}`;
              //console.log(`【京东账???${$.index}???${$.nickName} 换取京豆失败???${$.data.data.bizMsg}`)
              return
            }
            if ($.data.data.bizCode === 0) {
              if (`${coinToBeans}` === '1000') {
                $.beanscount ++;
                console.log(`【京东账???${$.index}???${$.nickName} ???${$.data.data.result.exchangeNum}次换${$.title}成功`)
...
function msgShow() {
  ...
  return new Promise(async resolve => {
    $.log(`\n【京东账???${$.index}???${$.nickName}\n${coinToBeans ? `【兑???${$.title}???${$.beanscount ? `成功` : $.beanerr}` : "您设置的是不兑换奖品"}\n`);
    let ctrTemp;
    ...
    if ($.beanscount && ctrTemp) {
      $.msg($.name, ``, `【京东账???${$.index}???${$.nickName}\n${coinToBeans ? `【兑???${$.title}???${ $.beanscount ? `成功，数量：${$.beanscount}个` : $.beanerr}` : "您设置的是不兑换奖品"}`);
      if ($.isNode()) {
        await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}`, `【京东账???${$.index}???${$.UserName}\n${coinToBeans ? `【兑???${$.title}???${$.beanscount ? `成功，数量：${$.beanscount}个` : $.beanerr}` : "您设置的是不兑换奖品"}`)
      }
    }
    resolve()
  })
