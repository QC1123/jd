        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success) {
              ...
              else if (data.resultCode === '9200011') console.log('已经助力???')
              else if (data.resultCode === '2400205') console.log('团已???')
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.data.shareTaskRes) {
              console.log(`\n【京东账???${$.index}???${$.nickName || $.UserName}）的${$.name}好友互助码???${data.data.shareTaskRes.itemId}\n`);
            } else {
              console.log(`已满5人助???,暂时看不到您???${$.name}好友助力码`)
            }
          }
        }
      } catch (e) {
          ...
          if (safeGet(data)) {
            ...
            if (func === "doInteractTask") {
              if (data.subCode === "S000") {
                console.log(`任务完成，获??? ${data.data.taskDetailResList[0].incomeAmountConf} 金币???${data.data.taskDetailResList[0].beanNum} 京豆`)
