...
京东家庭???
活动入口：玩一???-家庭???
8000幸福值可???100京豆，一天任务做完大???300幸福值，周期较长
已支持IOS双京东账???,Node.js支持N个京东账???
脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
============Quantumultx===============
[task_local]
#京东家庭???
1 * * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_family.js, tag=京东家庭???, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_family.png, enabled=true

================Loon==============
[Script]
cron "1 * * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_family.js,tag=京东家庭???

===============Surge=================
京东家庭??? = type=cron,cronexp="1 * * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_family.js

============小火???=========
京东家庭??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_family.js, cronexpr="1 * * * *", timeout=3600, enable=true
 */
const $ = new Env('京东家庭???');
        ...
        if (err) {
          ...
          if (info) {
            ...
            if (vo.length > 0) {
              ...
              // 5fed97ce5da81a8c069810dd 做美??? 40 10 2
              // 5fed97ce5da81a8c069810dc 去组??? 150 13 5
              if (vo['isdo'] === 1) {
                if (vo['times'] === 0) {
                  ...
                  console.log(`${Math.trunc(vo['times'] / 60)}分钟可后做任???${task['_id']}`)
                }
              }
            }
          }
