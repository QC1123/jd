 ...
 */
/*
最近经常出现给偷好友积分与狗粮失败的情况，故建议cron设置为多???
jd宠汪汪偷好友积分与狗???,及给好友喂食
偷好友积分上限是20个好???(即获???100积分)，帮好友喂食上限???20个好???(即获???200积分)，偷好友狗粮上限也是20个好???(最多获???120g狗粮)
IOS用户支持京东双账???,NodeJs用户支持N个京东账???
脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
如果开启了给好友喂食功能，建议先凌???0点运行jd_joy.js脚本获取狗粮后，再运行此脚本(jd_joy_steal.js)可偷好友积分???6点运行可偷好友狗???
==========Quantumult X==========
[task_local]
#宠汪汪偷好友积分与狗???
0 0-10/2 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy_steal.js, tag=宠汪汪偷好友积分与狗???, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true

=======Loon========
[Script]
cron "0 0-10/2 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy_steal.js,tag=宠汪汪偷好友积分与狗???

========Surge==========
宠汪汪偷好友积分与狗??? = type=cron,cronexp="0 0-10/2 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy_steal.js

=======小火???=====
宠汪汪偷好友积分与狗??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy_steal.js, cronexpr="0 0-10/2 * * *", timeout=3600, enable=true
*/

const $ = new Env('宠汪汪偷好友积分与狗???');
...
let jdJoyHelpFeed = false;//是否给好友喂食，false为不给喂食，true为给好友喂食，默认不给好友喂???
let jdJoyStealCoin = true;//是否偷好友积分与狗粮，false为否，true为是，默认是???
      ...
      if ($.getFriendsData && $.getFriendsData.datas && $.getFriendsData.datas.length  > 0) {
        ...
        for (let i = 1; i <= new Array(lastPage).fill('').length; i++) {
          if ($.visit_friend >= 100 || $.stealFriendCoin * 1 >= 100) {
            console.log('偷好友积分已达上???(已获???100积分) 跳出\n')
            $.stealFriendCoin = `已达上限(已获???100积分)`;
            break
          }
          console.log(`偷好友积??? 开始查询第${i}页好友\n`);
          await getFriends(i);
        ...
        for (let i = 1; i <= new Array(lastPage).fill('').length; i++) {
          if ($.help_feed >= 200 || ($.helpFeedStatus && $.helpFeedStatus === 'chance_full')) {
            console.log('帮好友喂食已达上???(已帮???20个好友获???200积分) 跳出\n');
            $.helpFood = '已达上限(已帮???20个好友获???200积分)'
            break
          }
          ...
          if (!$.HelpFeed) {
            console.log('您已设置不为好友喂食，现在跳过喂食，如需为好友喂食请在BoxJs打开喂食开关或者更改脚??? jdJoyHelpFeed ???');
            break
          }
          console.log(`帮好友喂??? 开始查询第${i}页好友\n`);
          await getFriends(i);
          $.allFriends = $.getFriendsData.datas;
  ...
  if (jdJoyStealCoinTemp) {
    if ($.visit_friend !== 100) {
      ...
      for (let friends of $.allFriends) {
        ...
        await stealFriendCoin(friendPin);//领好友积???
        if ($.stealFriendCoin * 1 === 100) {
          console.log(`偷好友积分已达上???${$.stealFriendCoin}个，现跳出循环`)
          break
        }
      }
    } else {
      console.log('偷好友积分已达上???(已获???100积分)')
      $.stealFriendCoin = `已达上限(已获???100积分)`
    }
  }
}
...
async function helpFriendsFeed() {
  if ($.help_feed !== 200) {
      ...
      for (let friends of $.allFriends) {
        ...
        if (status === 'not_feed') {
          ...
          if (helpFeedRes && helpFeedRes.errorCode === 'help_ok' && helpFeedRes.success) {
            console.log(`帮好友[${friendPin}]喂食10g狗粮成功,你获???10积分\n`);
            if (!ctrTemp) {
              $.log('为完成为好友单独喂食一次的任务，故此处进行喂食一???')
              $.HelpFeed = false;
              break
            }
            $.helpFood += 10;
          ...
          console.log(`帮好友喂食失???,好友[${friendPin}]的汪汪正在食用\n`)
        }
      }
    } else {
      console.log('您已设置不为好友喂食，现在跳过喂食，如需为好友喂食请在BoxJs打开喂食开关或者更改脚??? jdJoyHelpFeed ???')
    ...
    console.log('帮好友喂食已达上???(已帮???20个好友获???200积分)')
    $.helpFood = '已达上限(已帮???20个好友获???200积分)'
  }
}
...
function showMsg() {
  return new Promise(resolve => {
    $.stealFood = $.stealFood >= 0 ? `【偷好友狗粮】获???${$.stealFood}g狗粮\n` : `【偷好友狗粮???${$.stealFood}\n`;
    $.stealFriendCoin = $.stealFriendCoin >= 0 ? `【领取好友积分】获???${$.stealFriendCoin}个\n` : `【领取好友积分???${$.stealFriendCoin}\n`;
    $.helpFood = $.helpFood >= 0 ? `【给好友喂食】消???${$.helpFood}g狗粮,获得积分${$.helpFood}个\n` : `【给好友喂食???${$.helpFood}\n`;
