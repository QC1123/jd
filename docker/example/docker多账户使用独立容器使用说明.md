...
#### `docker-compose.yml`文件参???
```yaml
version: "3"
services:
  jd_scripts1: #默认
    image: lxk0301/jd_scripts
    # 配置服务器资源约束。此例子中服务被限制为使用内存不超过200M以及cpu不超??? 0.2（单核的20%???
    # 经过实际测试，建议不低于200M
    # deploy:
    #   resources:
    #     limits:
