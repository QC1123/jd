...
[task_local]
#签到领现金
2 0 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_cash.js, tag=签到领现???, img-url=https://raw.githubusercontent.com/Orz-3/task/master/jd.png, enabled=true

================Loon==============
[Script]
cron "2 0 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_cash.js,tag=签到领现???

===============Surge=================
签到领现??? = type=cron,cronexp="2 0 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_cash.js

============小火???=========
签到领现??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_cash.js, cronexpr="2 0 * * *", timeout=3600, enable=true
 */
const $ = new Env('签到领现???');
...
async function helpFriends() {
  ...
  for (let code of $.newShareCodes) {
    console.log(`去帮助好???${code['inviteCode']}`)
  ...
  //     console.log(`去帮助好???${helpInfo['inviteCode']}`)
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if( data.code === 0 && data.data.bizCode === 0){
              console.log(`助力成功，获???${data.data.result.cashStr}`)
