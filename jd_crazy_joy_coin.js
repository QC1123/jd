      ...
      try {
        if (err) {
          ...
          if (data.success && data.data && data.data.shop) {
            ...
            $.buyJoyLevel = shop.length ? shop[shop.length - 1]['joyId'] : 1;//可购买的最大等???
            if ($.isNode() && process.env.BUY_JOY_LEVEL) {
              $.log(`当前可购买的最高JOY等级???${$.buyJoyLevel}级\n`)
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data.newJoyId) {
              console.log(`合并成功，获???${data.data.newJoyId}级Joy`)
            } else
              console.log(`合并失败，错误`)
          }
        }
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.success)
              console.log(`金币补给领取成功，获???${data.data.coins}金币`)
          }
        }
      } catch (e) {
        $.logErr(e, resp)
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.data && data.data.tryMoneyJoyBeans) {
              console.log(`分红狗生效中，预计获??? ${data.data.tryMoneyJoyBeans} 京豆奖励`)
            }
            ...
            if (data.data) {
              $.log(`此次在线收益：获??? ${data.data['coins']} 金币`)
            }
          }
        }
      } catch (e) {
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            // data = JSON.parse(data);
            // if (data.data && data.data.tryMoneyJoyBeans) {
            //   console.log(`【京东账??? ${userName}】分红狗生效中，预计获得 ${data.data.tryMoneyJoyBeans} 京豆奖励`)
            // }
            // if (data.data) {
            //   $.log(`【京东账??? ${userName}】此次在线收益：获得 ${data.data['coins']} 金币`)
            // }
          }
        }
      } catch (e) {
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data['success']) {
              $.log(`点击幸运盒子成功，剩余观看视频次数：${data.data.advertViewTimes}, ${data.data.advertViewTimes > 0 ? '等待32???' : '跳出'}`)
