...
[task_local]
#宠汪汪积分兑换奖品
0 0-16/8 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy_reward.js, tag=宠汪汪积分兑换奖???, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true

==============Loon==============
[Script]
cron "0 0-16/8 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy_reward.js,tag=宠汪汪积分兑换奖???

================Surge===============
宠汪汪积分兑换奖??? = type=cron,cronexp="0 0-16/8 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy_reward.js

===============小火???==========
宠汪汪积分兑换奖??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy_reward.js, cronexpr="0 0-16/8 * * *", timeout=3600, enable=true
 */
const $ = new Env('宠汪汪积分兑换奖???');
let joyRewardName = 20;//是否兑换京豆，默认开启兑换功能，其中20为兑???20京豆,500为兑???500京豆???0为不兑换京豆.数量有限先到先得
//Node.js用户请在jdCookie.js处填写京东ck;
const jdCookieNode = $.isNode() ? require('./jdCookie.js') : '';
const notify = $.isNode() ? require('./sendNotify') : '';
let jdNotify = false;//是否开启静默运行，默认false关闭(???:奖品兑换成功后会发出通知提示)
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      console.log(`本地时间与京东服务器时间???(毫秒)???${await get_diff_time()}`);
      await joyReward();
      // $.msg($.name, '兑换脚本暂不能使???', `请停止使用，等待后期更新\n如果新版本兑换您有兑换机会，请抓包兑换\n再把抓包数据发送telegram用户@LXK9301`);
    }
  }
})()
    .catch((e) => {
  ...
  if ($.getExchangeRewardsRes && $.getExchangeRewardsRes.success) {
    ...
    if (rewardNum && (rewardNum === 1 || rewardNum === 20 || rewardNum === 50 || rewardNum === 100 || rewardNum === 500 || rewardNum === 1000)) {
      ...
      if (data.coin >= salePrice) {
        ...
        console.log(`兑换${rewardNum}京豆失败，原因：您目前只???${data.coin}积分，已不足兑换${giftValue}京豆所需???${salePrice}积分\n`)
        //$.msg($.name, `兑换${giftName}失败`, `【京东账???${$.index}???${$.nickName}\n目前只有${data.coin}积分\n已不足兑???${giftName}所需???${salePrice}积分\n`)
      }
    } else {
      console.log('您设置了不兑换京???,如需兑换京豆，请去BoxJs重新设置或修改第20行代???')
    }
  } else {
