/*
东东水果:脚本更�地址 https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_fruit.js
更新时间???2021-1-9
...
//助力好友分享???(最???4???,否则后面的助力失???),原因:京东农场每人每天只有四次助力机会
...
let jdFruitBeanCard = false;//农场使用水滴换豆???(如果出现限时活动???100g水换20???,此时比浇水划???,推荐换豆),true表示换豆(不浇???),false表示不换???(继续浇水),脚本默认是浇???
...
async function jdFruit() {
  ...
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      console.log(`\n【京东账???${$.index}???${$.nickName || $.UserName}）的${$.name}好友互助码???${$.farmInfo.farmUserPro.shareCode}\n`);
      console.log(`\n【已成功兑换水果???${$.farmInfo.farmUserPro.winTimes}次\n`);
      message += `【已兑换水果???${$.farmInfo.farmUserPro.winTimes}次\n`;
      await masterHelpShare();//助力好友
      if ($.farmInfo.treeState === 2 || $.farmInfo.treeState === 3) {
        option['open-url'] = urlSchema;
        $.msg($.name, ``, `【京东账???${$.index}???${$.nickName || $.UserName}\n【提醒⏰???${$.farmInfo.farmUserPro.name}已可领取\n请去京东APP或微信小程序查看\n点击弹窗即达`, option);
        if ($.isNode()) {
          await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}水果已可领取`, `【京东账???${$.index}???${$.nickName || $.UserName}\n【提醒⏰???${$.farmInfo.farmUserPro.name}已可领取\n请去京东APP或微信小程序查看`);
        }
        return
      } else if ($.farmInfo.treeState === 1) {
        console.log(`\n${$.farmInfo.farmUserPro.name}种植???...\n`)
      ...
      await getWaterFriendGotAward();//领取???2好友浇水奖励
      await duck();
      await doTenWaterAgain();//再次浇水
      await predictionFruit();//预测水果成熟时间
    } else {
      console.log(`初始化农场数据异???, 请登录京??? app查看农场0元水果功能是否正???,农场初始化数???: ${JSON.stringify($.farmInfo)}`);
      message = `【京东账???${$.index}??? ${$.nickName || $.UserName}\n【数据异常】请手动登录京东app查看此账???${$.name}是否正常`;
    }
  } catch (e) {
    console.log(`任务执行异常，请检查执行日??? ‼️‼️`);
    message = `任务执行异常，请检查执行日??? ‼️‼️`;
    $.logErr(e);
  }
  await showMsg();
}
async function doDailyTask() {
  ...
  if (!$.farmTask.signInit.todaySigned) {
    ...
    if ($.signResult.code === "0") {
      console.log(`【签到成功】获???${$.signResult.amount}g💧\\n`)
      //message += `【签到成功】获???${$.signResult.amount}g💧\n`//连续签到${signResult.signDay}???
    } else {
      // message += `签到失败,详询日志\n`;
      console.log(`签到结果:  ${JSON.stringify($.signResult)}`);
    }
  } else {
    console.log(`今天已签???,连续签到${$.farmTask.signInit.totalSigned},下次签到可得${$.farmTask.signInit.signEnergyEachAmount}g\n`);
  }
  // 被水滴砸???
  console.log(`被水滴砸中： ${$.farmInfo.todayGotWaterGoalTask.canPop ? '???' : '???'}`);
  if ($.farmInfo.todayGotWaterGoalTask.canPop) {
    await gotWaterGoalTaskForFarm();
    if ($.goalResult.code === '0') {
      console.log(`【被水滴砸中】获???${$.goalResult.addEnergy}g💧\\n`);
      // message += `【被水滴砸中】获???${$.goalResult.addEnergy}g💧\n`
    }
  }
  ...
  if (!$.farmTask.gotBrowseTaskAdInit.f) {
    ...
    let browseSuccess = 0
    let browseFail = 0
    for (let advert of adverts) { //开始浏览广???
    ...
    if (browseFail > 0) {
      console.log(`【广告浏览】完???${browseSuccess}???,失败${browseFail},获得${browseReward}g💧\\n`);
      // message += `【广告浏览】完???${browseSuccess}???,失败${browseFail},获得${browseReward}g💧\n`;
    } else {
      console.log(`【广告浏览】完???${browseSuccess}???,获得${browseReward}g💧\n`);
      // message += `【广告浏览】完???${browseSuccess}???,获得${browseReward}g💧\n`;
    }
  } else {
  ...
  if (!$.farmTask.gotThreeMealInit.f) {
    ...
    if ($.threeMeal.code === "0") {
      console.log(`【定时领水】获???${$.threeMeal.amount}g💧\n`);
      // message += `【定时领水】获???${$.threeMeal.amount}g💧\n`;
    } else {
      // message += `【定时领水】失???,详询日志\n`;
  ...
  // await Promise.all([
  //   clockInIn(),//打卡领水
  //   executeWaterRains(),//水滴???
  //   masterHelpShare(),//助力好友
  //   getExtraAward(),//领取额外水滴奖励
  //   turntableFarm()//天天抽奖得好???
  // ])
  await getAwardInviteFriend();
  await clockInIn();//打卡领水
  await executeWaterRains();//水滴???
  await getExtraAward();//领取额外水滴奖励
  await turntableFarm()//天天抽奖得好???
}
async function predictionFruit() {
  console.log('开始预测水果成熟时间\n');
  await initForFarm();
  await taskInitForFarm();
  let waterEveryDayT = $.farmTask.totalWaterTaskInit.totalWaterTaskTimes;//今天到到目前为止，浇了多少次???
  message += `【今日共浇水???${waterEveryDayT}次\n`;
  message += `【剩??? 水滴???${$.farmInfo.farmUserPro.totalEnergy}g💧\n`;
  message += `【水果🍉进度???${(($.farmInfo.farmUserPro.treeEnergy / $.farmInfo.farmUserPro.treeTotalEnergy) * 100).toFixed(2)}%，已浇水${$.farmInfo.farmUserPro.treeEnergy / 10}???,还需${($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy) / 10}次\n`
  ...
  // 预测n天后水果课可兑换功能
  let waterTotalT = ($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy - $.farmInfo.farmUserPro.totalEnergy) / 10;//一共还需浇多少次???

  let waterD = Math.ceil(waterTotalT / waterEveryDayT);

  message += `【预测???${waterD === 1 ? '明天' : waterD === 2 ? '后天' : waterD + '天之???'}(${timeFormat(24 * 60 * 60 * 1000 * waterD + Date.now())}???)可兑换水果🍉`
}
//浇水十次
async function doTenWater() {
  ...
  if (`${jdFruitBeanCard}` === 'true' && JSON.stringify($.myCardInfoRes).match(`限时翻倍`) && beanCard > 0) {
    console.log(`您设置的是使用水滴换豆卡，且背包有水滴换豆卡${beanCard}???, 跳过10次浇水任务`)
    return
  }
    ...
    for (; waterCount < $.farmTask.totalWaterTaskInit.totalWaterTaskLimit - $.farmTask.totalWaterTaskInit.totalWaterTaskTimes; waterCount++) {
      ...
      if ($.waterResult.code === '0') {
        ...
        if ($.waterResult.finished) {
          // 已证实，waterResult.finished为true，表示水果可以去领取兑换???
...
async function getFirstWaterAward() {
  ...
  if (!$.farmTask.firstWaterInit.f && $.farmTask.firstWaterInit.totalWaterTimes > 0) {
    ...
    if ($.firstWaterReward.code === '0') {
      console.log(`【首次浇水奖励】获???${$.firstWaterReward.amount}g💧\n`);
      // message += `【首次浇水奖励】获???${$.firstWaterReward.amount}g💧\n`;
    } else {
      // message += '【首次浇水奖励】领取奖励失???,详询日志\n';
...
async function getTenWaterAward() {
  ...
  if (!$.farmTask.totalWaterTaskInit.f && $.farmTask.totalWaterTaskInit.totalWaterTaskTimes >= $.farmTask.totalWaterTaskInit.totalWaterTaskLimit) {
    ...
    if ($.totalWaterReward.code === '0') {
      console.log(`【十次浇水奖励】获???${$.totalWaterReward.totalWaterTaskEnergy}g💧\n`);
      // message += `【十次浇水奖励】获???${$.totalWaterReward.totalWaterTaskEnergy}g💧\n`;
    } else {
      // message += '【十次浇水奖励】领取奖励失???,详询日志\n';
      console.log(`领取10次浇水奖励结???:  ${JSON.stringify($.totalWaterReward)}`);
    }
  } else if ($.farmTask.totalWaterTaskInit.totalWaterTaskTimes < $.farmTask.totalWaterTaskInit.totalWaterTaskLimit) {
    // message += `【十次浇水奖励】任务未完成，今日浇???${$.farmTask.totalWaterTaskInit.totalWaterTaskTimes}次\n`;
    console.log(`【十次浇水奖励】任务未完成，今日浇???${$.farmTask.totalWaterTaskInit.totalWaterTaskTimes}次\n`);
  }
...
async function doTenWaterAgain() {
  ...
  await myCardInfoForFarm();
  const { fastCard, doubleCard, beanCard, signCard  } = $.myCardInfoRes;
  console.log(`背包已有道具:\n快速浇水卡:${fastCard === -1 ? '未解???': fastCard + '???'}\n水滴翻倍卡:${doubleCard === -1 ? '未解???': doubleCard + '???'}\n水滴换京豆卡:${beanCard === -1 ? '未解???' : beanCard + '???'}\n加签???:${signCard === -1 ? '未解???' : signCard + '???'}\n`)
  ...
  if (signCard > 0) {
    ...
    for (let i = 0; i < new Array(signCard).fill('').length; i++) {
      ...
      console.log(`使用加签卡结???:${JSON.stringify($.userMyCardRes)}`);
    }
    await initForFarm();
    totalEnergy = $.farmInfo.farmUserPro.totalEnergy;
  }
  ...
  if (`${jdFruitBeanCard}` === 'true' && JSON.stringify($.myCardInfoRes).match('限时翻???')) {
    console.log(`\n您设置的是水滴换豆功???,现在为您换豆`);
    if (totalEnergy >= 100 && $.myCardInfoRes.beanCard > 0) {
      //使用水滴换豆???
      await userMyCardForFarm('beanCard');
      console.log(`使用水滴换豆卡结???:${JSON.stringify($.userMyCardRes)}`);
      if ($.userMyCardRes.code === '0') {
        message += `【水滴换豆卡】获???${$.userMyCardRes.beanCount}个京豆\n`;
        return
      }
    } else {
      console.log(`您目前水???:${totalEnergy}g,水滴换豆???${$.myCardInfoRes.beanCard}???,暂不满足水滴换豆的条???,为您继续浇水`)
    }
  }
  ...
  //   totalEnergy  = $.farmInfo.farmUserPro.totalEnergy;
  // }
  // 所有的浇水(10次浇???)任务，获取水滴任务完成后，如果剩余水滴大于等???60g,则继续浇???(保留部分水滴是用于完成第二天的浇???10次的任务)
  let overageEnergy = totalEnergy - retainWater;
  if (totalEnergy >= ($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy)) {
    //如果现有的水滴，大于水果可兑换所需的对???(也就是把水滴浇完，水果就能兑换了)
    isFruitFinished = false;
    for (let i = 0; i < ($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy) / 10; i++) {
      await waterGoodForFarm();
      console.log(`本次浇水结果(水果马上就可兑换???):   ${JSON.stringify($.waterResult)}`);
      if ($.waterResult.code === '0') {
        console.log('\n浇水10g成功\n');
        if ($.waterResult.finished) {
          // 已证实，waterResult.finished为true，表示水果可以去领取兑换???
    ...
    console.log("目前剩余水滴：???" + totalEnergy + "】g,不再继续浇水,保留部分水滴用于完成第二天【十次浇水得水滴】任???")
  }
}
//领取阶段性水滴奖???
function gotStageAward() {
  return new Promise(async resolve => {
    if ($.waterResult.waterStatus === 0 && $.waterResult.treeEnergy === 10) {
      console.log('果树发芽???,奖励30g水滴');
      await gotStageAwardForFarm('1');
      console.log(`浇水阶段奖励1领取结果 ${JSON.stringify($.gotStageAwardForFarmRes)}`);
      if ($.gotStageAwardForFarmRes.code === '0') {
        // message += `【果树发芽了】奖???${$.gotStageAwardForFarmRes.addEnergy}\n`;
        console.log(`【果树发芽了】奖???${$.gotStageAwardForFarmRes.addEnergy}\n`);
      }
      ...
      if ($.gotStageAwardForFarmRes.code === '0') {
        // message += `【果树开花了】奖???${$.gotStageAwardForFarmRes.addEnergy}g💧\n`;
        console.log(`【果树开花了】奖???${$.gotStageAwardForFarmRes.addEnergy}g💧\n`);
      }
      ...
      if ($.gotStageAwardForFarmRes.code === '0') {
        // message += `【果树结果了】奖???${$.gotStageAwardForFarmRes.addEnergy}g💧\n`;
        console.log(`【果树结果了】奖???${$.gotStageAwardForFarmRes.addEnergy}g💧\n`);
      }
    }
    resolve()
  })
...
async function turntableFarm() {
  ...
  if ($.initForTurntableFarmRes.code === '0') {
    ...
    if ($.initForTurntableFarmRes.turntableBrowserAds && $.initForTurntableFarmRes.turntableBrowserAds.length > 0) {
      for (let index = 0; index < $.initForTurntableFarmRes.turntableBrowserAds.length; index++) {
        if (!$.initForTurntableFarmRes.turntableBrowserAds[index].status) {
          console.log(`开始浏览天天抽奖的???${index + 1}个逛会场任务`)
    ...
    //天天抽奖助力
    console.log('开始天天抽???--好友助力--每人每天只有三次助力机会.')
    ...
    if (remainLotteryTimes > 0) {
      ...
      for (let i = 0; i < new Array(remainLotteryTimes).fill('').length; i++) {
        ...
        console.log(`???${i + 1}次抽奖结???${JSON.stringify($.lotteryRes)}`);
...
async function getExtraAward() {
  ...
  if ($.masterHelpResult.code === '0') {
    if ($.masterHelpResult.masterHelpPeoples && $.masterHelpResult.masterHelpPeoples.length >= 5) {
      ...
      message += `【额外奖励】领取失???,原因：给您助力的人未???5个\n`;
    }
    if ($.masterHelpResult.masterHelpPeoples && $.masterHelpResult.masterHelpPeoples.length > 0) {
      ...
      $.masterHelpResult.masterHelpPeoples.map((item, index) => {
        ...
        console.log(`\n京东昵称???${item.nickName || "匿名用户"}??? ??? ${time} 给您助过力\n`);
      })
      message += `【助力您的好友???${str}\n`;
    }
...
async function masterHelpShare() {
  ...
  for (let code of newShareCodes) {
    ...
    if ($.helpResult.code === '0') {
      if ($.helpResult.helpResult.code === '0') {
        ...
        console.log(`给好友???${$.helpResult.helpResult.masterUserInfo.nickName}】助力获???${$.helpResult.helpResult.salveHelpAddWater}g水滴`)
  ...
  if (salveHelpAddWater > 0) {
    // message += `【助力好友👬】获???${salveHelpAddWater}g💧\n`;
    console.log(`【助力好友👬】获???${salveHelpAddWater}g💧\n`);
  }
...
async function executeWaterRains() {
  ...
  if (executeWaterRain) {
    console.log(`水滴雨任务，每天两次，最多可???10g水滴`);
    console.log(`两次水滴雨任务是否全部完成：${$.farmTask.waterRainInit.f ? '???' : '???'}`);
    if ($.farmTask.waterRainInit.lastTime) {
      if (Date.now() < ($.farmTask.waterRainInit.lastTime + 3 * 60 * 60 * 1000)) {
        executeWaterRain = false;
        // message += `【第${$.farmTask.waterRainInit.winTimes + 1}次水滴雨】未到时间，???${new Date($.farmTask.waterRainInit.lastTime + 3 * 60 * 60 * 1000).toLocaleTimeString()}再试\n`;
        console.log(`\`【第${$.farmTask.waterRainInit.winTimes + 1}次水滴雨】未到时间，???${new Date($.farmTask.waterRainInit.lastTime + 3 * 60 * 60 * 1000).toLocaleTimeString()}再试\n`);
      }
    }
    if (executeWaterRain) {
      console.log(`开始水滴雨任务,这是???${$.farmTask.waterRainInit.winTimes + 1}次，剩余${2 - ($.farmTask.waterRainInit.winTimes + 1)}次`);
      await waterRainForFarm();
      console.log('水滴雨waterRain');
      if ($.waterRain.code === '0') {
        console.log('水滴雨任务执行成功，获得水滴???' + $.waterRain.addEnergy + 'g');
        console.log(`【第${$.farmTask.waterRainInit.winTimes + 1}次水滴雨】获???${$.waterRain.addEnergy}g水滴\n`);
        // message += `【第${$.farmTask.waterRainInit.winTimes + 1}次水滴雨】获???${$.waterRain.addEnergy}g水滴\n`;
      }
    }
  } else {
    // message += `【水滴雨】已全部完成，获???20g💧\n`;
  }
}
//打卡领水活动
  ...
  if ($.clockInInit.code === '0') {
    ...
    if (!$.clockInInit.todaySigned) {
      ...
      if ($.clockInForFarmRes.code === '0') {
        // message += `【第${$.clockInForFarmRes.signDay}天签到】获???${$.clockInForFarmRes.amount}g💧\n`;
        console.log(`【第${$.clockInForFarmRes.signDay}天签到】获???${$.clockInForFarmRes.amount}g💧\n`)
        if ($.clockInForFarmRes.signDay === 7) {
          //可以领取惊喜礼包
          console.log('开始领???--惊喜礼包38g水滴');
          await gotClockInGift();
          if ($.gotClockInGiftRes.code === '0') {
            // message += `【惊喜礼包】获???${$.gotClockInGiftRes.amount}g💧\n`;
            console.log(`【惊喜礼包】获???${$.gotClockInGiftRes.amount}g💧\n`);
          }
        }
      }
    }
        ...
        if (!item.hadGot) {
          ...
          if ($.themeStep1.code === '0') {
            ...
            if ($.themeStep2.code === '0') {
              console.log(`关注${item.name}，获得水???${$.themeStep2.amount}g`);
            }
          }
        }
      }
        ...
        if (!item.hadGot) {
          ...
          if ($.venderCouponStep1.code === '0') {
            ...
            if ($.venderCouponStep2.code === '0') {
              ...
              console.log(`???${item.name}领券，获得水???${$.venderCouponStep2.amount}g`);
            }
          }
        }
      }
...
async function getAwardInviteFriend() {
  ...
  if ($.friendList) {
    console.log(`\n今日已邀请好???${$.friendList.inviteFriendCount}??? / 每日邀请上???${$.friendList.inviteFriendMax}个`);
    console.log(`开始删???${$.friendList.friends && $.friendList.friends.length}个好???,可拿每天的邀请奖励`);
    if ($.friendList.friends && $.friendList.friends.length > 0) {
      for (let friend of $.friendList.friends) {
        console.log(`\n开始删除好??? [${friend.shareCode}]`);
    ...
    await receiveFriendInvite();//为他人助???,接受邀请成为别人的好友
...
async function doFriendsWater() {
  ...
  if (waterFriendCountKey < waterFriendMax) {
    ...
    if ($.friendList.friends && $.friendList.friends.length > 0) {
      ...
      //TODO ,发现bug,github action运行发现有些账号第一次没有给3个好友浇???
      ...
      for (let index = 0; index < needWaterFriends.length; index ++) {
        ...
        console.log(`为第${index+1}个好友浇水结???:${JSON.stringify($.waterFriendForFarmRes)}\n`)
        if ($.waterFriendForFarmRes.code === '0') {
          waterFriendsCount ++;
          if ($.waterFriendForFarmRes.cardInfo) {
            console.log('为好友浇水获得道具了');
            if ($.waterFriendForFarmRes.cardInfo.type === 'beanCard') {
              console.log(`获取道具???:${$.waterFriendForFarmRes.cardInfo.rule}`);
              cardInfoStr += `水滴换豆???,`;
      ...
      // message += `【好友浇水】已???${waterFriendsCount}个好友浇???,消???${waterFriendsCount * 10}g水\n`;
      console.log(`【好友浇水】已???${waterFriendsCount}个好友浇???,消???${waterFriendsCount * 10}g水\n`);
    ...
    console.log(`今日已为好友浇水量已???${waterFriendMax}个`)
  }
}
//领取???3个好友浇水后的奖励水???
async function getWaterFriendGotAward() {
  ...
  if (waterFriendCountKey >= waterFriendMax) {
    if (!waterFriendGotAward) {
      ...
      console.log(`领取???${waterFriendMax}个好友浇水后的奖励水???::${JSON.stringify($.waterFriendGotAwardRes)}`)
      if ($.waterFriendGotAwardRes.code === '0') {
        // message += `【给${waterFriendMax}好友浇水】奖???${$.waterFriendGotAwardRes.addWater}g水滴\n`;
        console.log(`【给${waterFriendMax}好友浇水】奖???${$.waterFriendGotAwardRes.addWater}g水滴\n`);
      }
    } else {
      console.log(`给好友浇水的${waterFriendSendWater}g水滴奖励已领取\n`);
      // message += `【给${waterFriendMax}好友浇水】奖???${waterFriendSendWater}g水滴已领取\n`;
    }
  } else {
    console.log(`暂未???${waterFriendMax}个好友浇水\n`);
  }
}
//接收成为对方好友的邀???
async function receiveFriendInvite() {
  for (let code of newShareCodes) {
    ...
    // console.log(`接收邀请成为好友结???:${JSON.stringify($.inviteFriendRes.helpResult)}`)
    if ($.inviteFriendRes.helpResult.code === '0') {
      console.log(`接收邀请成为好友结果成???,您已成为${$.inviteFriendRes.helpResult.masterUserInfo.nickName}的好友`)
    } else if ($.inviteFriendRes.helpResult.code === '17') {
      console.log(`接收邀请成为好友结果失???,对方已是您的好友`)
    }
  }
  // console.log(`开始接???6fbd26cc27ac44d6a7fed34092453f77的邀请\n`)
...
async function duck() {
  for (let i = 0; i < 10; i++) {
    ...
    if ($.duckRes.code === '0') {
      if (!$.duckRes.hasLimit) {
        console.log(`小鸭子游???:${$.duckRes.title}`);
        // if ($.duckRes.type !== 3) {
