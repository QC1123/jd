...
更新时间???2020-12-24
建议先凌???0点运行jd_joy.js脚本获取狗粮后，再运行此脚本(jd_joy_steal.js)可偷好友积分???6点运行可偷好友狗???
feedCount:自定??? 每次喂养数量; 等级只和喂养次数有关，与数量无关
推荐每次投喂10个，积累狗粮，然后去玩聚宝盆???
Combine from Zero-S1/JD_tools(https://github.com/Zero-S1/JD_tools)
==========Quantumult X==========
[task_local]
#京东宠汪???
15 0-23/2 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy.js, tag=京东宠汪???, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true

============Loon===========
[Script]
cron "15 0-23/2 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy.js,tag=京东宠汪???

============Surge==========
[Script]
京东宠汪??? = type=cron,cronexp="15 0-23/2 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy.js

===============小火???==========
京东宠汪??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_joy.js, cronexpr="15 0-23/2 * * *", timeout=3600, enable=true
*/
...
let teamLevel = `2`;//参加多少人的赛跑比赛，默认是双人赛跑，可???2???10,50。其他不可选，其中2代表参加双人PK赛，10代表参加10人突围赛???50代表参加50人挑战赛，如若想设置不同账号参加不同类别的比赛则???&区分即可(如：`2&10&50`)
//是否参加宠汪汪双人赛跑（据目前观察，参加双人赛跑不消耗狗???,如需参加其他多人赛跑，请关闭???
...
async function joinTwoPeopleRun() {
  ...
  if (`${joyRunFlag}` === 'true') {
    ...
    await getPetRace();
    console.log(`\n===以下是京东账???${$.index} ${$.nickName} ${$.petRaceResult.data.teamLimitCount || teamLevelTemp}人赛跑信???===\n`)
    if ($.petRaceResult) {
      ...
      if (petRaceResult === 'not_participate') {
        console.log(`暂未参赛，现在为您参???${teamLevelTemp}人赛跑`);
      ...
      if (petRaceResult === 'unbegin') {
        console.log('比赛还未开始，请九点再???');
      }
      if (petRaceResult === 'time_over') {
        console.log('今日参赛的比赛已经结束，请明天九点再???');
      }
      ...
      if (petRaceResult === 'participate') {
        ...
        if($.raceUsers && $.raceUsers.length > 0) {
          for (let index = 0; index < $.raceUsers.length; index++) {
            if (index === 0) {
              console.log(`您当前里程：${$.raceUsers[index].distance}KM\n当前排名:???${$.raceUsers[index].rank}名\n将获得积???:${$.raceUsers[index].coin}\n`);
  ...
  for (let item of $.getPetTaskConfigRes.datas) {
    ...
    if (item['taskType'] === 'SignEveryDay') {
      if (item['receiveStatus'] === 'chance_left') {
        console.log('每日签到未完???,需要自己手动去微信小程序【来客有礼】签到，可获得京豆奖???')
    ...
    if (item['taskType'] === 'HelpFeed') {
      if (item['receiveStatus'] === 'chance_left') {
        ...
        console.log(`领取每日帮好友喂一次狗??? 狗粮结果???${res.data}`);
      }
    }
    //每日喂狗???
    ...
    if (item['taskType'] === 'InviteUser') {
      if (item['receiveStatus'] === 'chance_left') {
        console.log('未完???,需要自己手动去邀请好友给你助???,可以获得狗粮')
      } else if (item['receiveStatus'] === 'unreceive') {
    ...
    if (item['taskType'] === 'ScanMarket') {
      ...
      for (let scanMarketItem of scanMarketList) {
        if (!scanMarketItem.status) {
          ...
          console.log(`逛会???-${scanMarketItem.marketName}结果::${JSON.stringify(scanMarketRes)}`)
        }
      }
    }
    //浏览频道
          ...
          if (data.success) {
            if (data.errorCode === 'feed_ok') {
              ...
              if ((feedNum) !== 0) {
                ...
                console.log('您的狗粮已不???10g')
                message += `【喂食失败】您的狗粮已不足10g\n`;
              }
            } else {
              console.log(`其他状???${data.errorCode}`)
            }
          }
        }
      } catch (e) {
...
function runMatch(teamLevel, timeout = 5000) {
  ...
  console.log(`正在参赛中，请稍???${timeout / 1000}秒，以防多个账号匹配到统一赛场\n`)
    ...
    if ($.getSupplyInfoRes.data) {
      ...
      for (let list of marketList) {
        if (!list['status']) {
          ...
          $.log(`能力补给??? ${$.getSupplyInfoRes.data.addDistance}km里程 已领取\n`);
          status = list['status'];
        }
      }
      if (!status) {
