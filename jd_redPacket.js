...
async function redPacket() {
  ...
  if (response.code === 0) {
    ...
    if ($.taskInfo && $.taskInfo.length > 0) {
      console.log(`    任务     状???  红包是否领取`);
      for (let item of $.taskInfo) {
        console.log(`${item.title.slice(-6)}   ${item.alreadyReceivedCount ? item.alreadyReceivedCount: 0}/${item.requireCount}      ${item.innerStatus === 4 ? '???':'???'}`)
      }
      for (let item of $.taskInfo) {
        //innerStatus=4已领取红包，3：任务已完成，红包未领取???2：任务未完成???7,未领取任???
    ...
    if (getTaskDetailForColorRes.data && getTaskDetailForColorRes.data.result) {
      ...
      for (let item of advertDetails) {
        ...
        if (item.id && item.status == 0) {
          ...
          // console.log(`完成任务的动???---${JSON.stringify(taskReportForColorRes)}`)
        }
      }
    } else {
      console.log(`任务列表为空,手动进入app内检??? 是否存在[从京豆首页进领券中心???30秒]的任???,如存???,请手动完成再运行脚本`)
      $.msg(`${$.name}`, '', '手动进入app内检查\n是否存在[从京豆首页进领券中心???30秒]的任务\n如存???,请手动完成再运行脚本');
