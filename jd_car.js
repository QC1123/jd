...
更新�址：https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_car.js
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.status) {
              console.log(`签到成功，获???${data.data.point}，已签到${data.data.signDays}天`)
              message += `签到成功，获???${data.data.point}，已签到${data.data.signDays}天\n`
            }
          }
        }
      } catch (e) {
