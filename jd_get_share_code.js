...
更新�址：https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_get_share_code.js
已支持IOS双京东账号, Node.js支持N个京东账号
脚本兼容: QuantumultX, Surge, Loon, 小火箭，JSBox, Node.js
============Quantumultx===============
[task_local]
#获取互助码
20 13 * * 6 https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_get_share_code.js, tag=获取互助???, img-url=https://raw.githubusercontent.com/Orz-3/task/master/jd.png, enabled=true

================Loon==============
[Script]
cron "20 13 * * 6" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_get_share_code.js, tag=获取互助???

===============Surge=================
获取互助??? = type=cron,cronexp="20 13 * * 6",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_get_share_code.js

============小火???=========
获取互助??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_get_share_code.js, cronexpr="20 13 * * 6", timeout=3600, enable=true
 */
const $ = new Env("获取互助???");
            ...
            if (safeGet(data)) {
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
                    console.log(
                      `【账???${$.index}???${$.nickName || $.UserName}）东东工厂???${item.assistTaskDetailVo.taskToken}`
                    );
                  }
                });
              }
        ...
        try {
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                data = data["data"];
                $.unActive = true; //标记是否开启了京喜活动或者选购了商品进行生???
                $.encryptPin = "";
                $.shelvesList = [];
                if (data.factoryList && data.productionList) {
                  ...
                  console.log(`【账???${$.index}???${$.nickName || $.UserName}）京喜工厂???${data.user.encryptPin}`);
                }
              } else {
                $.unActive = false; //标记是否开启了京喜活动或者选购了商品进行生???
                if (!data.factoryList) {
                  console.log(
                    `【提示】京东账???${$.index}[${$.nickName}]京喜工厂活动未开始请手动去京东APP->游戏与互???->查看更多->京喜工厂 开启活动`
                  );
                } else if (data.factoryList && !data.productionList) {
                  console.log(
                    `【提示】京东账???${$.index}[${$.nickName}]京喜工厂未选购商品请手动去京东APP->游戏与互???->查看更多->京喜工厂 选购`
                  );
                }
              }
            } else {
        ...
        try {
          if (err) {
            ...
            if (safeGet(data)) {
              data = JSON.parse(data);
              if (data["ret"] === 0) {
                console.log(`【账???${$.index}???${$.nickName || $.UserName}）京喜农场助力码???${data.smp}`);

                if (data.active) {
                  console.log(`【账???${$.index}???${$.nickName || $.UserName}）京喜农场active???${data.active}`);
                } else {
                  console.log( `【账???${$.index}???${$.nickName || $.UserName}）京喜农场】未选择种子，请先去京喜农场选择种子`);
                }
              }
            } else {
        ...
        if (err) {
          ...
          if (
            ...
            if ($.petInfo.userStatus === 0) {
              /*console.log(
                `【提示】京东账???${$.index}${$.nickName}萌宠活动未开启请手动去京东APP开启活动入口：我的->游戏与互???->查看更多开启`
              );*/
              return;
            }

            console.log(
              `【账???${$.index}???${$.nickName || $.UserName}）京东萌宠???${$.petInfo.shareCode}`
            );
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.data.shareTaskRes) {
                console.log(`【账???${$.index}???${$.nickName || $.UserName}）京东赚赚???${data.data.shareTaskRes.itemId}`);
              } else {
                //console.log(`已满5人助???,暂时看不到您的京东赚赚好友助力码`)
              }
            }
          }
        } catch (e) {
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0") {
      ...
      console.log(`【账???${$.index}???${$.nickName || $.UserName}）种豆得豆???${$.myPlantUuid}`);
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      console.log(
        `【账???${$.index}???${$.nickName || $.UserName}）京东农场???${$.farmInfo.farmUserPro.shareCode}`
      );

    } else {
      /*console.log(
        `初始化农场数据异???, 请登录京??? app查看农场0元水果功能是否正???,农场初始化数???: ${JSON.stringify(
          $.farmInfo
        )}`
      );*/
    }
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data && data.data.userInviteCode) {
              console.log(`【账???${$.index}???${$.nickName || $.UserName}）crazyJoy???${data.data.userInviteCode}`)
            }
          }
        }
      } catch (e) {
