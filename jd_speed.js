/*
京东天天加速�接：https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_speed.js
更新时间???2020-12-25
支持京东双账???
脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
每天4京豆，再小的苍蝇也是???
??? https://github.com/Zero-S1/JD_tools/blob/master/JD_speed.py 改写来的
建议3小时运行一次，打卡时间间隔???6小时
=================QuantumultX==============
[task_local]
#天天加???
8 0-23/3 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_speed.js, tag=京东天天加???, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdjs.png, enabled=true

============Loon================
[Script]
cron "8 0-23/3 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_speed.js,tag=京东天天加???

===========Surge============
天天加??? = type=cron,cronexp="8 0-23/3 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_speed.js

==============小火???=============
天天加??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_speed.js, cronexpr="11 0-23/3 * * *", timeout=3600, enable=true
*/
...
let jdNotify = true;//是否开启静默运行。默认true开???
...
function jDSpeedUp(sourceId) {
  return new Promise((resolve) => {
    ...
    $.get(url, async (err, resp, data) => {
      try {
        if (err) {
          console.log('京东天天-加???: 签到接口请求失败 ‼️‼️');
          ...
          if (data) {
            ...
            if (res.info.isLogin === 1) {
              ...
              if (res.data.task_status === 0) {
                ...
                const step1 = await energyPropList();//检查燃???
                const step2 = await receiveEnergyProp(step1);//领取可用的燃???
                ...
                if (data.match(/\"beans_num\":\d+/)) {
                  //message += "【上轮奖励】成功领???" + data.match(/\"beans_num\":(\d+)/)[1] + "京豆 🐶";
                  if (!jdNotify || jdNotify === 'false') {
                    $.msg($.name, '', `【京东账???${$.index}???${$.nickName}\n` + "【上轮太空旅行】成功领???" + data.match(/\"beans_num\":(\d+)/)[1] + "京豆 🐶");
                  }
                } else {
                  console.log("京东天天-加???: 成功, 明细: 无京??? 🐶")
                }
                console.log("\n天天加???-领取上次奖励成功")
                await jDSpeedUp(null);
              } else {
        ...
        $.msg("京东天天-加???" + e.name + "‼️", JSON.stringify(e), e.message)
      } finally {
        resolve()
      }
    })
  ...
  return new Promise((resolve) => {
    ...
    $.get(spaceEventUrl, async (err, resp, data) => {
      try {
          ...
          if (data) {
            ...
            if (cc.message === "success" && cc.data.length > 0) {
              ...
              if (spaceEvents && spaceEvents.length > 0) {
                console.log("\n天天加???-查询???" + spaceEvents.length + "个太空特殊事???")
              } else {
                console.log("\n天天加???-暂无太空特殊事件")
              }
            } else {
        ...
        $.msg("天天加???-查询太空特殊事件" + e.name + "‼️", JSON.stringify(e), e.message)
      } finally {
        resolve(spaceEvents)
      }
    })
    ...
    if (spaceEventList && spaceEventList.length > 0) {
      ...
      for (let item of spaceEventList) {
        ...
        $.get(spaceHandleUrl, (err, resp, data) => {
          try {
            if (err) {
              console.log("\n京东天天-加???: 处理太空特殊事件请求失败 ‼️‼️")
              console.log(`${JSON.stringify(err)}`)
            } else {
              if (data) {
                const cc = JSON.parse(data);
                // console.log(`处理特殊事件的结果：???${JSON.stringify(cc)}`);
                console.log("\n天天加???-尝试处理???" + spaceEventCount + "个太空特殊事???")
            ...
            $.msg("天天加???-查询处理太空特殊事件" + e.name + "‼️", JSON.stringify(e), e.message)
          } finally {
            if (spaceEventList.length === spaceNumTask) {
              console.log("\n天天加???-已成功处???" + spaceNumTask + "个太空特殊事???")
              resolve()
            }
          }
        })
  ...
  return new Promise((resolve) => {
    ...
    $.get(QueryUrl, async (err, resp, data) => {
      try {
        ...
        $.msg("天天加???-查询燃料" + eor.name + "‼️", JSON.stringify(eor), eor.message)
      } finally {
        resolve(TaskID)
      }
    })
      ...
      for (let i = 0; i < CID.length; i++) {
        ...
        $.get(TUrl, (error, response, data) => {
          try {
            ...
            $.msg("天天加???-领取可用燃料" + eor.name + "‼️", JSON.stringify(eor), eor.message)
          } finally {
            if (CID.length === count) {
              console.log("\n天天加???-已成功领???" + NumTask + "个可用燃???")
              resolve(NumTask)
            }
          }
        })
  ...
  return new Promise((resolve) => {
    ...
    $.get(EUrl, (error, response, data) => {
      try {
        ...
        $.msg("天天加???-燃料ID" + eor.name + "‼️", JSON.stringify(eor), eor.message)
      } finally {
        resolve(TaskCID)
      }
    })
      ...
      for (let i = 0; i < PropID.length; i++) {
        ...
        $.get(PropUrl, (error, response, data) => {
          try {
            ...
            $.msg("天天加???-使用燃料" + eor.name + "‼️", JSON.stringify(eor), eor.message)
          } finally {
            if (PropID.length === PropCount) {
              console.log("\n天天加???-已成功使???" + PropNumTask + "个燃???")
              resolve(PropNumTask)
            }
          }
        })
