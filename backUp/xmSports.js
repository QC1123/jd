...
小米运动修改微信支付宝运动步???
APP Store下载小米运动APP
登入小米运动(登录方式必须是手机号???+密码(没有就用手机号码注册),下面的第三方账号(小米账号,Apple,微信)授权登录不行)
登录成功后在 我的->第三方接???->绑定支付???,微信
小米运动只要不退出登录，就会自动获取新的token,即永久有???
[MITM]
...
http-response ^https:\/\/account\.huami\.com\/v2\/client\/login script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/backUp/xmSports.js, requires-body=true, timeout=3600, tag=小米运动获取Token
 */
...
let login_token = '';
//需要修改的运动步数波动范围，脚本默认修改步数范围为1w9???2w5
