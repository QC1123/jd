/*
京东天天加速活动 国内gitee�接：https://gitee.com/lxk0301/jd_scripts/raw/master/jd_speed.js
更新时间:2020-08-15
每天4京豆，再小的苍蝇也是肉
...
// [task_local]
// #天天加???
// 8 */3 * * * https://gitee.com/lxk0301/jd_scripts/raw/master/jd_speed.js, tag=京东天天加???, img-url=https://raw.githubusercontent.com/znz1992/Gallery/master/jdttjs.png, enabled=true
// Loon
// [Script]
// cron "8 */3 * * *" script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_speed.js,tag=京东天天加???
const name = '天天加???';
const $ = new Env(name);
...
function energyPropList() {
  ...
  request('energyProp_list', body).then(response => {
    // console.log(`检查燃料列???:${JSON.stringify(response)}`);
  ...
  request(functionId, body).then((res) => {
    ...
    if (res.code === 0) {
      ...
      if (data.beans_num) {
        ...
        task_status = data.task_status //0,没开始；1，已开???
