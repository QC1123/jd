...
let shareCodes = [ // 这个列表填入你要助力的好友的shareCode, 最多可能是5???
...
//按顺序执???, 尽量先执行不消耗狗粮的任务, 避免中途狗粮不???, 而任务还没做???
let function_map = {
  ...
  browseShopsInit: browseShopsInit, //浏览店铺s, 目前只有一个店???
  firstFeedInit: firstFeedInit, //首次喂食
  inviteFriendsInit: inviteFriendsInit, //邀请好???, 暂未处理
  feedReachInit: feedReachInit, //喂食10次任???  最后执行投???10次任???, 提示剩余狗粮是否够投???10次完成任???, 并询问要不要继续执行
}
...
async function feedReachInit() {
  ...
  // if (confirm('当前剩余狗粮' + foodAmount + 'g, 已不足投???' + needFeedTimes + '???, 确定要继续吗?') === false) {
  ...
  for (let code of shareCodes) {
    ...
    if (response.code === '0' && response.resultCode === '0') {
      if (response.result.helpStatus === 0) {
        console.log('已给好友: ???' + response.result.masterNickName + '】助???');
...
function browseSingleShopInit2() {
  console.log('准备浏览指定店铺--冰淇淋会???');
  const body = {"index":1,"version":1,"type":1};
  const body2 = {"index":1,"version":1,"type":2}
  request("getSingleShopReward", body).then(response => {
    console.log(`①点击浏览指定店铺结???: ${JSON.stringify(response)}`);
    if (response.code === '0' && response.resultCode === '0') {
      request("getSingleShopReward", body2).then(response2 => {
        console.log(`②浏览指定店铺结???: ${JSON.stringify(response2)}`);
        if (response2.code === '0' && response2.resultCode === '0') {
          message += `【冰淇淋会场】获取狗???${response2.result.reward}g\n`;
        }
      })
    }
    gen.next();
  ...
  request("getSingleShopReward", body).then(response => {
    ...
    if (response.code === '0' && response.resultCode === '0') {
      request("getSingleShopReward", body2).then(response2 => {
        console.log(`②浏览指定店铺结???: ${JSON.stringify(response2)}`);
        if (response2.code === '0' && response2.resultCode === '0') {
          message += `【去参与星品解锁计划】获取狗???${response2.result.reward}g\n`;
        }
      })
    }
    gen.next();
...
function initPetTown() {
  request(arguments.callee.name.toString()).then((response) => {
    ...
    if (response.code === '0' && response.resultCode === '0' && response.message === 'success') {
      ...
      console.log(`初始化萌宠失???:  ${response.message}`);
...
async function feedPetsAgain() {
  const response = await secondInitPetTown(); //再次初始化萌???
  if (response.code === '0' && response.resultCode === '0' && response.message === 'success') {
    ...
    if (foodAmount - 100 >= 10) {
      ...
      console.log("目前剩余狗粮：???" + foodAmount + "】g,不再继续投食,保留100g用于完成第二天任???");
    ...
    request('getInviteFriendsReward').then((res) => {
      try {
        if (res.code == 0 && res.resultCode == 0) {
          ...
          message += `【邀请新用户】获???${taskInfo.inviteFriendsInit.reward}g\n`;
        }
        gen.next();
      } catch (e) {
...
function taskInit() {
  ...
  request(arguments.callee.name.toString(), body).then(response => {
    ...
    // console.log(`任务初始化完???: ${JSON.stringify(taskInfo)}`);
    gen.next();
  })
