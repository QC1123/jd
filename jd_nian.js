...
暂不加入品牌会员
活�入???:https://wbbny.m.jd.com/babelDiy/Zeus/2cKMj86srRdhgWcKonfExzK4ZMBy/index.html
活动地址：京东app左侧浮动窗口
...
const inviteCodes = [
  `cgxZdDjRf-uZqUKFawD4juwn6wbTMpI@cgxZczjGfeuEqVaMVknljFEc40bAC_-qM-bAPlKO@cgxZdTXtIL-MvljPClSu6EFYVzoxq2DpsvRkeZlFMZP5emzkiv6juYaPMHM@cgxZdTXtU_m4tFiUcH3gsNZWk5oVClunQhZrDbXC0DMjykp3SwtwXfc`,
  ``
];
!(async () => {
  await requireConfig();
      ...
      try {
        if (err) {
          ...
          if (data && data.data['bizCode'] === 0) {
            ...
            console.log(`当前爆竹${$.userInfo.raiseInfo.remainScore}🧨，下一关需???${$.userInfo.raiseInfo.nextLevelScore}🧨`)
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.data.bizCode === 0) {
             console.log(`收取成功，获???${data.data.result.produceScore}爆竹🧨`)
              // $.userInfo = data.data.result.userInfo;
            }
          }
        }
            ...
            if (data.code === 0) {
              if (data.data && data.data.bizCode === 0) {
                if(data.data.result.score)
                  console.log(`任务完成，获???${data.data.result.score}爆竹🧨`)
          ...
          if (safeGet(data)) {
            ...
            if(data.code===0 && data.data && data.data.result) {
              ...
              if(result.awardType===4)
                console.log(`抽奖成功，获???${result.score}爆竹🧨`)
              else if(result.awardType===2 || result.awardType===3)
                console.log(`抽奖成功，获得优惠卷`)
              else
                console.log(`抽奖成功，获???${JSON.stringify(result)}`)
            }else{
              console.log(`抽奖失败`)
            }
          }
          ...
          if (safeGet(data)) {
            ...
            if (data.data && data.data.bizCode === 0) {
              ...
              for(let vo of data.data.result){
                if(!vo.status){
                  console.log(`去领???${vo['productName']}优惠券`)
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.data && data.data.bizCode === 0) {
              console.log(`领取成功，获???${data.data.result.score}爆竹🧨`)
            }else{
              console.log(data.data.bizMsg)
            }
          }
