...
脚本会给内置的码进行助力
活动于2020-12-14日结束
活动�址：https://h5.m.jd.com/babelDiy/Zeus/2zwQnu4WHRNfqMSdv69UPgpZMnE2/index.html/
已支持IOS双京东账号,Node.js支持N个京东账号
脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
============Quantumultx===============
[task_local]
#苹果抽奖机
10 0 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_apple_live.js, tag=苹果抽奖???, enabled=true

================Loon==============
[Script]
cron "10 0 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_apple_live.js,tag=苹果抽奖???

===============Surge=================
苹果抽奖??? = type=cron,cronexp="10 0 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_apple_live.js

============小火???=========
苹果抽奖??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_apple_live.js, cronexpr="10 0 * * *", timeout=3600, enable=true
 */
const $ = new Env('苹果抽奖???');
...
async function jdHealth() {
  ...
  if($.userInfo.scorePerLottery<=$.userInfo.userScore){
    console.log(`当前用户金币: ${$.userInfo.userScore}，抽奖需???${$.userInfo.scorePerLottery}，开始抽奖`)
    message += `当前用户金币: ${$.userInfo.userScore}，抽奖需???${$.userInfo.scorePerLottery}，开始抽奖\n`
    for(let i=0;i<parseInt($.userInfo.userScore)/$.userInfo.scorePerLottery;++i){
      await jdapple_getLottery()
    }
  }else{
    console.log(`当前用户金币: ${$.userInfo.userScore}，抽奖需???${$.userInfo.scorePerLottery}，金币不足无法抽奖`)
    message += `当前用户金币: ${$.userInfo.userScore}，抽奖需???${$.userInfo.scorePerLottery}，金币不足无法抽奖\n`
  }
  await showMsg();
}
