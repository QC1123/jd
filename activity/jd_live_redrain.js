...
更新�址：https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_live_redrain.js
已支持IOS双京东账号, Node.js支持N个京东账号
脚本兼容: QuantumultX, Surge, Loon, 小火箭，JSBox, Node.js
============Quantumultx===============
[task_local]
#直播红包雨
0 0,9,11,13,15,17,19,20,21,23 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_live_redrain.js, tag=直播红包???, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_redPacket.png, enabled=true

================Loon==============
[Script]
cron "0 0,9,11,13,15,17,19,20,21,23 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_live_redrain.js, tag=直播红包???

===============Surge=================
直播红包??? = type=cron,cronexp="0 0,9,11,13,15,17,19,20,21,23 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_live_redrain.js

============小火???=========
直播红包??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_live_redrain.js, cronexpr="0 0,9,11,13,15,17,19,20,21,23 * * *", timeout=3600, enable=true
 */
const $ = new Env('直播红包???');
let ids = {
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.isLogin = true;
      $.nickName = '';
      message = `???${new Date().getUTCHours()+8}???${$.name}】`
      await TotalBean();
...
async function showMsg() {
  if ($.isNode() && !jdNotify) {
    await notify.sendNotify(`【京东账???${$.index}???${$.nickName}`, message)
  }
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.subCode === '0') {
              console.log(`领取成功，获???${JSON.stringify(data.lotteryResult)}`)
              // message+= `领取成功，获???${JSON.stringify(data.lotteryResult)}\n`
              message += `领取成功，获??? ${(data.lotteryResult.jPeasList[0].quantity)} 京豆\n`
