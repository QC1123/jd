        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              message += `【签到钞票】：收取成功，获??? ${data['data']['rewardMoneyToday']}\n`
            } else {
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret']===0){
              ...
              console.log(`翻倍成功，获得${ratio}%，共计获???${data['data']['earnRatio']}%`)
            }
          }
        }
      } catch (e) {
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0&&data['data']['active']!=='') {
              console.log(`升级成功，获???${JSON.stringify(data['data'])}`)
            }
          }
        }
      } catch (e) {
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data['ret'] === 0) {
              console.log(`点击厂长成功，获??? ${data['data']['moneyNum']} 钞票`)
            }else if(data['ret'] === 2005){
              // 点击上限
              $.click = false
            }else{
              console.log(`点击厂长过快，休???25秒`)
              await $.wait(25000);
            }
          }
        }
