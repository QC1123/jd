...
脚本会给内置的码进行助力
活动于2020-12-13日结束
活动�址：https://starsingle.m.jd.com/static/index.html#/?fromChangeSkinNum=PUBG
          ...
          if (safeGet(data)) {
            ...
            for (let i = 0; i< vo.productList.length;++i){
              ...
              if(product.productStatus === 1) {
                ...
                console.log(`【浏览商品】浏览商??? ${product.productName}未领奖，去领奖`)
                await doTask(`starId=PUBG&type=product&id=${product.productId}&status=2`)
              } else{
            ...
            for (let i = 0; i< vo.shopList.length;++i){
              ...
              if(shop.shopStatus === 0 || shop.shopStatus === 1) {
                ...
                console.log(`【关注店铺】关注店??? ${shop.shopName}未领奖，去领奖`)
                await doTask(`starId=PUBG&type=shop&id=${shop.shopId}&status=2`)
              }else{
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 200){
              if(data.data.bean){
                console.log(`任务完成，获得京??? ${data.data.bean}`)
                message += `任务完成，获得京??? ${data.data.bean}\n`
                $.bean += data.data.bean
              } else{
                console.log(`任务领取完成`)
              }
