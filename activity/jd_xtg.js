 ...
 */
/**
 星推�脚??? https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_xtg.js
 星推官活动地址：https://prodev.m.jd.com/mall/active/3gSzKSnvrrhYushciUpzHcDnkYE3/index.html
 活动时间???2020???10???21??? 00:00:00-2020???11???11??? 23:59:59
 京豆先到先得！！！！！！！！！！???
 出现任务做完没领取的情况，就再运行一次脚???
 能做完所有的任务，包括自动抽???,脚本会给内置的shareId助力
 一???23个活动，耗时比较久，surge请加大timeout时间
 支持京东双账???
 脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
 // quantumultx
 [task_local]
 #京东星推???
 2 0 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_xtg.js, tag=京东星推???, enabled=true
 // Loon
 [Script]
 cron "2 0 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_xtg.js,tag=京东星推???
 // Surge
 京东星推??? = type=cron,cronexp=2 0 * * *,wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_xtg.js
 */
const $ = new Env('京东星推???');
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      console.log(`一???${starID.length}???${$.name}任务，耗时会很久，请提前知晓`)
...
async function JD_XTG() {
  ...
  if ($.homeData && $.homeData.code === 200) {
    ...
    for (let item1 of venueList) {
      console.log(`\n任务二：逛逛[${item1['venueName']}]-${item1['venueStatus']  !== 3 ? '' : '已做???'}`)
    ...
    for (let item2 of productList) {
      console.log(`\n任务三：逛逛[${item2['productName']}]-${item2['productStatus']  !== 3 ? '' : '已做???'}`)
