...
async function jdJdh() {
  ...
  await queryTask(15,"meetingplace") // 逛义诊会???
  await queryTask(18,"2951198") // 看名医直???
  await queryTask(17,"246147") //
  await queryTask(24, t) // 辟谣
  await doTask(22,42,`${new Date().getUTCFullYear()}-${new Date().getUTCMonth()+1}-${new Date().getUTCDate()}`) // 去打???
  await queryTask(20,"362451650500001") // 测一???
  await doTask(23,40,`${rand(10000, 20000)}`) // 走路，这个可以直接提示领奖结???
  // 以下两个需要开启家庭医生才能完???
  await doTask(null,50,`${rand(10000, 20000)}`) // 家庭医生走路
  await queryTask(17,"235741")  // 家庭医生资讯，这个可以不用开启直接完???
  await queryInviteHome()
  await showMsg()
}
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.data.ownerInfo.firstVisitChance){
              console.log(`首次访问成功，获??? ${data.data.ownerInfo.firstVisitChance}张汪汪卡`)
            }
          }
        }
      } catch (e) {
...
async function showMsg() {
  ...
  if ($.isNode() && !jdNotify) {
    await notify.sendNotify(`【京东账???${$.index}???${$.nickName} `, `???${$.name}???${message}`);
  } else {
