...
活动入口: 京东app-我的-神仙书院
活动�址：https://h5.m.jd.com//babelDiy//Zeus//4XjemYYyPScjmGyjej78M6nsjZvj//index.html?babelChannel=ttt9
...
function showMsg() {
  return new Promise(resolve => {
    message += `本次运行获得${$.earn}金币，当???${$.coin}金币`
    if (!jdNotify) {
      $.msg($.name, '', `${message}`);
    } else {
        ...
        if (err) {
          ...
          if (data && data['retCode'] === "200") {
            if(data.result.score)
              console.log(`任务完成成功，获???${data.result.score}金币`)
            else if(data.result.taskToken)
              console.log(`任务请求成功，等???${$.duration}秒`)
            else{
...
function requireConfig() {
  return new Promise(async resolve => {
    ...
    console.log(`您提供的地理位置信息???${JSON.stringify($.cor)}`)
  ...
  return new Promise(resolve => {
    try {
      ...
      $.get({
        ...
        console.log('自动获取经纬??? lat lng 失败，返回经纬度结果错误');
        resolve({});
      });
    } catch (e) {
      console.log('自动获取经纬??? lat lng 失败，触发异???');
      resolve({});
    }
  });
}
