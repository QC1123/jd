          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              if (help) {
                ...
                console.log(`帮助好友收取 ${data.data['CollectElectricity']} 电力，获??? ${data.data['loginPinCollectElectricity']} 电力`);
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              $.log(`下次集满收取${data.data.nextCollectDoubleFlag === 1 ? '???' : '不可'}双倍电力`)
              console.log(`发电机：当前 ${data.data.currentElectricityQuantity} 电力，最大??? ${data.data.maxElectricityQuantity} 电力`)
              if (data.data.nextCollectDoubleFlag === 1) {
                if (data.data.currentElectricityQuantity === data.data.maxElectricityQuantity && data.data.doubleElectricityFlag) {
                  ...
                  message += `【发电机电力】当??? ${data.data.currentElectricityQuantity} 电力，未达到收获标准\n`
                }
              } else {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              if (data.factoryList && data.productionList) {
                ...
                await GetCommodityDetails();//获取已选购的商品信???
                ...
                console.log(`\n【京东账???${$.index}???${$.nickName || $.UserName}）的${$.name}好友互助码???${data.user.encryptPin}`);
                ...
                if (!data.factoryList) {
                  console.log(`【提示】京东账???${$.index}[${$.nickName}]京喜工厂活动未开始\n请手动去京东APP->游戏与互???->查看更多->京喜工厂 开启活动\n`);
                  // $.msg($.name, '【提示???', `京东账号${$.index}[${$.nickName}]京喜工厂活动未开始\n请手动去京东APP->游戏与互???->查看更多->京喜工厂 开启活动`);
                } else if (data.factoryList && !data.productionList) {
                  console.log(`【提示】京东账???${$.index}[${$.nickName}]京喜工厂未选购商品\n请手动去京东APP->游戏与互???->查看更多->京喜工厂 选购\n`)
                  let nowTimes = new Date(new Date().getTime() + new Date().getTimezoneOffset()*60*1000 + 8*60*60*1000);
                  if (nowTimes.getHours() % 12 === 0) {
                    //如按每小时运行一次，则此处将一天推???2次提???
                    $.msg($.name, '提醒???', `京东账号${$.index}[${$.nickName}]京喜工厂未选择商品\n请手动去京东APP->游戏与互???->查看更多->京喜工厂 选择商品`);
...
//可获取开团后的团ID，如果团ID为空并且surplusOpenTuanNum>0，则可继续开???
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data['ret'] === 0) {
              // $.log(`\n开团情???:${data.data.tuanInfo.realTuanNum}/${data.data.tuanInfo.tuanNum}\n`)
            } else {
              console.log(`异常???${JSON.stringify(data)}`);
            }
          }
                  ...
                  for (let item2 of userInfo) {
                    if (item2.encryptPin === $.encryptPin) {
                      if (item2.receiveElectric && item2.receiveElectric > 0) {
                        ...
                        console.log(`开始领???${new Date(item2.joinTime * 1000).toLocaleString()}参加团主???${item2.nickName}】的奖励`)
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              if (isTuanLeader) {
                console.log(`开团奖???(团长)${data.data['electric']}领取成功`);
                message += `【开???(团长)奖励???${data.data['electric']}领取成功\n`;
...
async function exchangeProNotify() {
  ...
  for (let shel of $.shelvesList) {
    console.log(`${shel['name']}    ${shel['exchangeStatus'] === 1 ? '未兑???' : shel['exchangeStatus'] === 2 ? '已兑???' : '兑换超时'}`)
  ...
  if (exchangeEndTime) {
    //比如兑换(超时)截止时间???2020/12/8 09:20:04,现在时间???2020/12/6
    if (nowTimes < exchangeEndTime) {
      //还可以兑???
      // 一:在兑换超时这一???(2020/12/8 09:20:04)的前2小时内通知
      if ((exchangeEndTime - nowTimes) <= 3600000 * 2) {
        ...
        if ($.isNode()) await notify.sendNotify(`${$.name} - 京东账号${$.index} - ${$.nickName}`, `【京东账???${$.index}???${$.nickName}\n【生产商品???${$.productName}${(exchangeEndTime - nowTimes) / 60*60*1000}分钟后兑换超时\n【兑换截止时间???${$.exchangeEndTime}\n请速去京喜APP->首页->好物0元造进行兑换`, { url: jxOpenUrl })
      }
      //???:在兑换超时日期前的时间一天通知三次(2020/12/6 9,10,11???,以及???2020/12/7 9,10,11点各通知一???)
