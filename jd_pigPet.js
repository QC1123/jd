...
[task_local]
#京东金融养猪???
12 * * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_pigPet.js, tag=京东金融养猪???, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdyz.png, enabled=true

================Loon==============
[Script]
cron "12 * * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_pigPet.js, tag=京东金融养猪???

===============Surge=================
京东金融养猪??? = type=cron,cronexp="12 * * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_pigPet.js

============小火???=========
京东金融养猪??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_pigPet.js, cronexpr="12 * * * *", timeout=3600, enable=true
 */
            ...
            if (data.resultCode === 0) {
              if (data.resultData.resultCode === 0) {
                if (data.resultData.resultData && data.resultData.resultData.goods) {
                  ...
                  for (let item of data.resultData.resultData.goods) {
                    if (item.count >= 20) {
                      console.log(`10秒后开始喂???${item.goodsName}，当前数量为${item.count}g`)
                      await $.wait(10000);
          ...
          if (data) {
            ...
            if (data.resultCode === 0) {
              if (data.resultData.resultCode === 0) {
                ...
                if (!$.hasPig) {
                  console.log(`\n京东账号${$.index} ${$.nickName} 未开启养猪活???,请手动去京东金融APP开启此活动\n`)
                }
              } else {
