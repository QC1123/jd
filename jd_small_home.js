...
let isPurchaseShops = false;//是否一键加购商品到购物车，默认不加???
$.helpToken = [];
if ($.isNode()) {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if ($.newShareCodes.length > 1) {
        ...
        console.log(`\n${$.UserName} 去给自己的下一账号 ${decodeURIComponent($.newShareCodes[(i + 1) % $.newShareCodes.length]['cookie'].match(/pt_pin=(.+?);/) && $.newShareCodes[(i + 1) % $.newShareCodes.length]['cookie'].match(/pt_pin=(.+?);/)[1])}助力，助力码??? ${code}\n`)
  ...
  for (let item of $.taskList) {
    ...
    if (item.ssjjTaskInfo.type === 4) {
      ...
      for (let i = 0; i < new Array(item.ssjjTaskInfo.awardOfDayNum).fill('').length; i++) {
        await followShops('followShops', item.ssjjTaskInfo.id);//一键关注店???
    ...
    if (isPurchaseShops && item.ssjjTaskInfo.type === 9) {
      ...
      for (let i = 0; i < new Array(item.ssjjTaskInfo.awardOfDayNum).fill('').length; i++) {
        await followShops('purchaseCommodities', item.ssjjTaskInfo.id);//一键加购商???
        await queryDoneTaskRecord(item.ssjjTaskInfo.id, item.ssjjTaskInfo.type);
      }
    }
            ...
            if (data.head.code === 200) {
              if (data.body) {
                ...
                if ($.canBuyList[0].needWoB <= $.woB) {
                  ...
                  console.log(`\n兑换${$.canBuyList[0].jdBeanNum}京豆失败:当前wo???${$.woB}不够兑换所需???${$.canBuyList[0].needWoB}WO币`)
                  message += `【装饰领京豆】兑???${$.canBuyList[0].jdBeanNum}京豆失败,原因:WO币不够\n`;
                }
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.head.code === 200) {
              if (data.body) {
                console.log(`${functionID === 'followShops'? '一键关注店???': '一键加购商???'}结果???${data.head.msg}`);
                // message += `【限时连连看】成功，活动${awardWoB}WO币\n`;
              }
            }
          }
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.head.code === 200) {
              if (data.body) {
                if (data.body.id) {
                  console.log(`\n您的${$.name}shareCode(每天都是变化???):???${data.body.id}】\n`);
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.head.code === 200) {
              if (data.body) {
                console.log(`\n给好???${data.body.inviteId}:???${data.head.msg}】\n`)
              }
            } else {
