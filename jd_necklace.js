...
APP活动入口：“最新版本京东APP >领券中心/券后9.9>领点点券”页???
...
[task_local]
#点点???
10 0,20 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_necklace.js, tag=点点???, img-url=https://raw.githubusercontent.com/Orz-3/task/master/jd.png, enabled=true

================Loon==============
[Script]
cron "10 0,20 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_necklace.js,tag=点点???

===============Surge=================
点点??? = type=cron,cronexp="10 0,20 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_necklace.js

============小火???=========
点点??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_necklace.js, cronexpr="10 0,20 * * *", timeout=3600, enable=true
 */
const $ = new Env('点点???');
...
async function jd_necklace() {
  ...
  await sign();
  await necklace_homePage();
  // await necklace_exchangeGift(10);//自动兑换多少钱的无门槛红包，1000代表1???
  await showMsg();
}
function showMsg() {
...
async function doTask() {
  for (let item of $.taskConfigVos) {
    if (item.taskStage === 0) {
      ...
      console.log(`${item.taskName}任务已做???,奖励未领取`);
