...
/*
京东年货???
???5000金币开盲盒必中200-300京豆，任务做完每???1000???5天换一???
活动时间???2021???1???9???-2021???2???9???
已支持IOS双京东账???,Node.js支持N个京东账???
脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
============Quantumultx===============
[task_local]
#京东年货???
1 7 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_nh.js, tag=京东年货???, img-url=https://raw.githubusercontent.com/Orz-3/task/master/jd.png, enabled=true

================Loon==============
[Script]
cron "1 7 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_nh.js,tag=京东年货???

===============Surge=================
京东年货??? = type=cron,cronexp="1 7 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_nh.js

============小火???=========
京东年货??? = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_nh.js, cronexpr="1 7 * * *", timeout=3600, enable=true
 */
const $ = new Env('京东年货???');
            ...
            if (data.data) {
              ...
              if (!info) {
                ...
                for(let i of ['sign','mainActive','visitSku','allFollowShop','allAddSku','memberCard']){
                  ...
                  } else if(task.taskName ==='一键关注店???' || task.taskName ==='一键开???' // || task.taskName ==='一键加???'
                  ){
                    if (task.count < task.taskMax){
                      console.log(`去做${task.taskName}任务`)
                      let res = await getTaskInfo(task.taskType)
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.result && data.data) {
              console.log(`任务完成成功，获???${data.data.score}金币`)
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.result && data.data) {
              console.log(`抽奖成功，获??? ${data.data.drawInfo || '空气'}`)
              message += `抽奖成功，获??? ${data.data.drawInfo || '空气'}`
            } else {
              console.log(`任务完成失败，错误信息：${data.errorMessage}`)
            }
          }
