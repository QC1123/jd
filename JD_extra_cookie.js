...
===================
===================
使用方式：在代理软件配置好下方配置后，复??? https://home.m.jd.com/myJd/newhome.action 到浏览器打开 ，在个人中心自动获取 cookie???
若弹出成功则正常使用。否则继续再此页面继续刷新一下试???
===================
...
function GetCookie() {
  try {
    if ($request.headers && $request.url.indexOf("GetJDUserInfoUnion") > -1) {
      ...
      if (CV.match(/(pt_key=.+?pt_pin=|pt_pin=.+?pt_key=)/)) {
        ...
        if (updateCodkie) {
          ...
          CookieName = "【账???" + updateCookiesData.length + "???";
          tipPrefix = "首次写入京东";
        }
    ...
    $.write("", CacheKey);
    $.notify("写入京东Cookie失败", "", "已尝试清空历史Cookie, 请重??? ⚠️");
    console.log(
      `\n写入京东Cookie出现错误 ‼️\n${JSON.stringify(
        eor
